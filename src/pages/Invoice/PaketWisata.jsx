...
const PaketWisata = () => {
  ...
  useEffect(() => {
    dispatch(getOrderIdTour(id))
  }, [dispatch, id])

  useEffect(() => {
    setDefaultData(tourBooking?.[0])
  }, [tourBooking])
  ...
  return (
    <>
                ...
                (
                  <>
                    <dl className="grid sm:grid-cols-9 gap-x-1">
                      ...
                      <dd className="col-span-6 text-gray-500 mb-0">{formatDate(defaultData?.arrival_date)}</dd>
                    </dl>
                    <dl className="grid sm:grid-cols-9 gap-x-1">
                      <dt className="col-span-3 font-semibold text-gray-800">Tanggal Pulang:</dt>
                      <dd className="col-span-6 text-gray-500 mb-0">{formatDate(defaultData?.departure_date)}</dd>
                    </dl>
                    <dl className="grid sm:grid-cols-9 gap-x-1">
                      <dt className="col-span-3 font-semibold text-gray-800">Jumlah Orang:</dt>
                      <dd className="col-span-6 text-gray-500 mb-0">{defaultData?.amount} orang</dd>
                    </dl>
                  </>
                )
              }
                  ...
                  <>
                    <div className="grid grid-cols-3 sm:grid-cols-5 gap-2">
                      <div className="col-span-full sm:col-span-2">
                        ...
                        <p className="font-medium text-gray-800">{defaultData?.tour_package?.title}</p>
                      </div>
                      <div>
                        <h5 className="sm:hidden text-xs font-medium text-gray-500 uppercase">Jumlah</h5>
                        <p className="text-gray-800">{defaultData?.meal_count}x makan</p>
                      </div>
                      <div>
                        <h5 className="sm:hidden text-xs font-medium text-gray-500 uppercase">Harga</h5>
                        <p className="text-gray-800">{rupiah(defaultData?.tour_package?.price || 0)}</p>
                      </div>
                      <div>
                        <h5 className="sm:hidden text-xs font-medium text-gray-500 uppercase">Total</h5>
                        <p className="sm:text-end text-gray-800">{rupiah(defaultData?.tour_package?.price * defaultData?.meal_count || 0)}</p>
                      </div>
                    </div>
                  </>
                )
                ...
                (
                  <>
                    <div className="grid grid-cols-3 sm:grid-cols-5 gap-2">
                      ...
                      <div>
                        <h5 className="sm:hidden text-xs font-medium text-gray-500 uppercase">Jumlah</h5>
                        <p className="text-gray-800">{defaultData?.amount} orang</p>
                      </div>
                      <div>
                        <h5 className="sm:hidden text-xs font-medium text-gray-500 uppercase">Harga</h5>
                        <p className="text-gray-800">{rupiah(defaultData?.tour_package?.price || 0)}</p>
                      ...
                      <div>
                        <h5 className="sm:hidden text-xs font-medium text-gray-500 uppercase">Total</h5>
                        <p className="sm:text-end text-gray-800">{rupiah(defaultData?.total_price || 0)}</p>
                      </div>
                    </div>
                  </>
                )
            ...
            <div className="grid grid-cols-2 sm:grid-cols-1 gap-3 sm:gap-2">
              {
                ...
                (
                  <dl className="grid sm:grid-cols-5 gap-x-3">
                    ...
                    <dd className="col-span-2 text-gray-500">{rupiah(defaultData?.tour_package?.price * defaultData?.meal_count || 0)} per orang</dd>
                  </dl>
                )
              }
              <dl className="grid sm:grid-cols-5 gap-x-3">
      ...
      <div style={{ display: "none" }}>
        <PaketWisataPrint ref={componentRef} data={defaultData} />
      </div>
    </>
  )
}
