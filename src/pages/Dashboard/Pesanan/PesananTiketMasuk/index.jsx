...
import { deleteTicketBooking, getTicketBooking, ticketBookingSelector } from "../../../../store/ticketBookingSlice";
  ...
  return (
    <div className="flex gap-6 ms-5">
      ...
      <div>
        ...
        <p>Kendaraan: {rowData?.vehicle?.type}</p>
      </div>
    </div>
  );
};
...
const Index = () => {
  ...
  useEffect(() => {
    setDefaultData(ticketBooking != "Tidak ada data" ? ticketBooking : [])
  }, [ticketBooking])
  ...
  const handleDelete = async () => {
    setLoad(true)
    const res = await dispatch(deleteTicketBooking(idDelete))
    setOpenDelete(false)
    await dispatch(getTicketBooking())
    setLoad(false)
    ...
    <>
      <Panel
        header={
          <>
            <Breadcrumb>
              <Breadcrumb.Item>Daftar Pesanan Tiket Masuk</Breadcrumb.Item>
            </Breadcrumb>
          </>
        }
      >
