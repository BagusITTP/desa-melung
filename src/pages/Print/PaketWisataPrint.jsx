...
export default class PaketWisataPrint extends Component {
  render() {
    const { data } = this.props;
    return (
      <div className="max-w-[85rem] px-4 sm:px-6 lg:px-8 mx-auto py-4 sm:py-2">
          ...
          <div className="flex flex-col p-4 sm:p-10 bg-white rounded-xl">
            ...
            <div className="mt-8 grid md:grid-cols-2 gap-3">
              <div>
                ...
                <dl className="grid md:grid-cols-5 gap-x-1 mt-3">
                  <dt className="col-span-2 font-semibold text-gray-800">Status Pembayaran:</dt>
                  <dd className="col-span-3 text-gray-500 mb-0">{data?.payment_status}</dd>
                </dl>
              </div>

              <div className="md:text-end">
                <div className="grid grid-cols-2 md:grid-cols-1 md:gap-2 gap-2">
                  <dl className="grid md:grid-cols-9 gap-x-1 !mb-0">
                    ...
                    (
                      <>
                        <dl className="grid md:grid-cols-9 gap-x-1 !mb-0">
                          ...
                          <dd className="col-span-6 text-gray-500 mb-0">{formatDate(data?.arrival_date)}</dd>
                        </dl>
                        <dl className="grid md:grid-cols-9 gap-x-1 !mb-0">
                          <dt className="col-span-3 font-semibold text-gray-800">Tanggal Pulang:</dt>
                          <dd className="col-span-6 text-gray-500 mb-0">{formatDate(data?.departure_date)}</dd>
                        </dl>
                        <dl className="grid md:grid-cols-9 gap-x-1 !mb-0">
                          <dt className="col-span-3 font-semibold text-gray-800">Jumlah Orang:</dt>
                          <dd className="col-span-6 text-gray-500 mb-0">{data?.amount} orang</dd>
                        </dl>
                      </>
                    )
                  }
                      ...
                      <>
                        <div className="grid grid-cols-3 sm:grid-cols-5 gap-2">
                          <div className="col-span-full sm:col-span-2">
                            ...
                            <p className="font-medium text-gray-800">{data?.tour_package?.title}</p>
                          </div>
                          <div>
                            <h5 className="sm:hidden text-xs font-medium text-gray-500 uppercase">Jumlah</h5>
                            <p className="text-gray-800">{data?.meal_count}x makan</p>
                          </div>
                          <div>
                            <h5 className="sm:hidden text-xs font-medium text-gray-500 uppercase">Harga</h5>
                            <p className="text-gray-800">{rupiah(data?.tour_package?.price || 0)}</p>
                          </div>
                          <div>
                            <h5 className="sm:hidden text-xs font-medium text-gray-500 uppercase">Total</h5>
                            <p className="sm:text-end text-gray-800">{rupiah(data?.tour_package?.price * data?.meal_count || 0)}</p>
                          </div>
                        </div>
                      </>
                    )
                    ...
                    (
                      <>
                        <div className="grid grid-cols-3 sm:grid-cols-5 gap-2">
                          ...
                          <div>
                            <h5 className="sm:hidden text-xs font-medium text-gray-500 uppercase">Jumlah</h5>
                            <p className="text-gray-800">{data?.amount} orang</p>
                          </div>
                          <div>
                            <h5 className="sm:hidden text-xs font-medium text-gray-500 uppercase">Harga</h5>
                            <p className="text-gray-800">{rupiah(data?.tour_package?.price || 0)}</p>
                          ...
                          <div>
                            <h5 className="sm:hidden text-xs font-medium text-gray-500 uppercase">Total</h5>
                            <p className="sm:text-end text-gray-800">{rupiah(data?.total_price || 0)}</p>
                          </div>
                        </div>
                      </>
                    )
            ...
            <div className="mt-8 flex sm:justify-end">
              <div className="w-full max-w-2xl sm:text-end space-y-2">
                <div className="grid grid-cols-2 sm:grid-cols-1 sm:gap-2">
                  {
                    data?.arrival_date &&
                    (
                      <dl className="grid sm:grid-cols-5 gap-x-3 !mb-0">
                        <dt className="col-span-3 font-semibold text-gray-800">Sub Total:</dt>
                        <dd className="col-span-2 text-gray-500">{rupiah(data?.tour_package?.price * data?.meal_count || 0)} per orang</dd>
                      </dl>
                    )
                  }
                  <dl className="grid sm:grid-cols-5 gap-x-3 !mb-0">
